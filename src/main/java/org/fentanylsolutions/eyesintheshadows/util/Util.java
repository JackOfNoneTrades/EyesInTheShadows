...
public class Util {
    ...
    public static byte[] prependLengthToByteArray(byte[] byteArray, int length) {
        ...
        try {
            ...
            EyesInTheShadows.LOG.error("Failed to concatenate byte arrays!");
            e.printStackTrace();
            return null;
        }
    }
    ...
    public static void registerSpawnEgg(String name, int primaryColor, int secondaryColor) {
        Item itemSpawnEgg = new EyesMonsterPlacer(name, primaryColor, secondaryColor)
            .setUnlocalizedName("spawn_egg_" + name.toLowerCase())
            .setTextureName(EyesInTheShadows.MODID + ":spawn_egg");
